...
namespace Sales_and_Inventory_System__Gadgets_Shop_
{
    public partial class frmCustomers : Form
    {
        ...
        private void auto()
        {
            txtCustomerID.Text = "F-" + GetUniqueKey(6);
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Digite o nome", "Erro de entrada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtCustomerName.Focus();
                return;
            }
            ...
            if (txtAddress.Text == "")
            {
                MessageBox.Show("Digite o endereço", "Erro de entrada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtAddress.Focus();
                return;
            }
            if (txtCity.Text == "")
            {
                MessageBox.Show("Insira a cidade", "Erro de entrada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtCity.Focus();
                return;
            }
            ...
            {
                MessageBox.Show("Por favor, digite o número do contato.", "Erro de entrada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtContactNo.Focus();
                return;
            }
            ...
            try
            {
                auto();
                    ...
                    cmd.ExecuteReader();
                    MessageBox.Show("Guardado com sucesso", "Dados do funcionário", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("Deseja mesmo deletar o cadastro?", "Cadastro de funcionário", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == System.Windows.Forms.DialogResult.Yes)
                {
                    delete_records();
                }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtCustomerName.Text == "")
                {
                    MessageBox.Show("Por favor, digite o nome", "Erro de entrada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    txtCustomerName.Focus();
                    return;
                }
                ...
                cmd.Parameters.AddWithValue("@d8", txtNotes.Text);
                cmd.ExecuteReader();
                MessageBox.Show("Atualizado com sucesso", "Dados do funcionário", MessageBoxButtons.OK, MessageBoxIcon.Information);
                btnUpdate.Enabled = false;
                if (con.State == ConnectionState.Open)
                {
                    con.Close();
