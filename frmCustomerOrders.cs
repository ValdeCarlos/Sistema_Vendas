...
namespace Sales_and_Inventory_System__Gadgets_Shop_
{
    ...
    ConnectionString cs = new ConnectionString();
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            cmd = new SqlCommand("SELECT RTRIM(invoiceNo) as [Nº Fatura],RTRIM(InvoiceDate) as [Data Fatura],RTRIM(Invoice_Info.CustomerID) as [Cod Funcionário],RTRIM(CustomerName) as [Nome Funcionário],RTRIM(SubTotal) as [SubTotal],RTRIM(GrandTotal) as [Total Geral],RTRIM(TotalPayment) as [Valor Pago],RTRIM(PaymentDue) as [Troco],RTRIM(PaymentType) as [Tipo de Pagamento],RTRIM(Status) as [Status] from Invoice_Info,Customer where Invoice_Info.CustomerID=Customer.CustomerID and InvoiceDate between @d1 and @d2 and Customer.CustomerID='" + txtCustomerID.Text + "' order by InvoiceDate desc", con);
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
             //   GroupBox4.Visible = true;
                ...
                cmd = new SqlCommand("SELECT RTRIM(invoiceNo) as [Nº Fatura], RTRIM(InvoiceDate) as [Data Fatura], RTRIM(Customer.CustomerID) as [Cod Funcionário], RTRIM(CustomerName) as [Nome Funcionário], RTRIM(SubTotal) as [SubTotal], RTRIM(TotalPayment) as [Valor a pagar], RTRIM(PaymentDue) as [Troco], RTRIM(PaymentType) as [Tipo Pagamento], RTRIM(Status) as [Status], RTRIM(GrandTotal) as [TOTAL] FROM Invoice_Info INNER JOIN Customer ON Invoice_Info.CustomerID = Customer.CustomerID WHERE Customer.CustomerID = @CustomerID ORDER BY CustomerName, InvoiceDate", con);
