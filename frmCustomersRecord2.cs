...
namespace Sales_and_Inventory_System__Gadgets_Shop_
{
    public partial class frmCustomersRecord2 : Form
    {
        ...
        public void GetData()
        {
                ...
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM(CustomerID)as [ID Funcionário],RTRIM(Customername) as [Nome Funcionário],RTRIM(address) as [Endereço],RTRIM(city) as [Cidade],RTRIM(ContactNo) as [Contacto],RTRIM(ContactNo1) as [Contacto Alternativo],(email) as [Email],(notes) as [Notas] from Customer order by CustomerName", con);
        ...
        private void txtCustomers_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM(CustomerID)as [ID Funcionário],RTRIM(Customername) as [Nome Funcionário],RTRIM(address) as [Endereço],RTRIM(city) as [Cidade],RTRIM(ContactNo) as [Contacto],RTRIM(ContactNo1) as [Contacto Alternativo],(email) as [Email],(notes) as [Notas] from Customer where CustomerName like '" + txtCustomers.Text + "%' order by CustomerName", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "Customer");
                dataGridView1.DataSource = myDataSet.Tables["Customer"].DefaultView;
